...
const themes = {
  ...
  random: [] // akan diisi gabungan semua
};
...
function prepareSlot(list) {
  slotList.innerHTML = "";
  // Buat list panjang biar animasi mulus
  const extended = [];
  for (let i = 0; i < 12; i++) {
    extended.push(...list);
  }
  // Pastikan topik yang dipilih ada di akhir
...
function startSpinAnimation(finalTopic) {
  ...
  function animate(currentTime) {
    ...
    // Jarak total yang harus ditempuh
...
function selectMode(mode) {
  ...
  currentTopic = finalTopic; // simpan dulu
...
function startSpeaking() {
  ...
  speakingInterval = setInterval(() => {
    ...
    // Suara dramatis di 5 detik terakhir
